...
class Standarization{
  ...
  public:
    ...
    vector<double> calculate(){
        ...
        double totalMean = 0;
        double totalSum = 0;
        vector<double> standarizedLst;
        for(double i : test_input_list){
          totalSum = totalSum + i;
        }
        totalMean = totalSum/test_input_list.size();
        for(double j :test_input_list){
            double tempCalc = j- totalMean;
            varianceResultsqSum = varianceResultsqSum + std::pow(tempCalc, 2);
        }
        double variance = varianceResultsqSum / test_input_list.size();

        double standardDeviation = std::pow(variance, 0.5);
        for(double k : test_input_list){
          standarizedLst.push_back((k- totalMean) / standardDeviation);
        }
